...
export async function sendSessionSummary(sessionId: string) {
  ...
  const template = emailTemplates.sessionSummary({
    ...
    date,
    summary: session.tutor_notes || 'No summary provided.',
    nextSteps: session.next_steps || undefined,
    skillsWorkedOn: session.skills_covered || undefined,
  });
...
export async function sendWeeklyReport(reportId: string) {
  ...
  const template = emailTemplates.weeklyReport({
    ...
    masteryUpdates: (report.mastery_changes as { skill: string; from: string; to: string }[]) || [],
 ...
 */
export async function sendAtRiskAlert(studentUserId: string, reasons: string[]) {
  if (!isAdminConfigured()) return;
