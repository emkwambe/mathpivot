...
export default async function TutorSessionsPage() {
  ...
  const { data: tutorBookings } = await supabase
    .from("bookings")
    .select("id, start_at, end_at, status, student_user_id, tutor_user_id")
    .eq("tutor_user_id", user.id);

  const bookingIds = tutorBookings?.map((b) => b.id) || [];

  // Get sessions for those bookings
  ...
  // Attach booking data to each session
  const bookingMap = new Map(tutorBookings?.map((b) => [b.id, b]) || []);
  const sessions =
    rawSessions?.map((s) => ({
      ...s,
  ...
  const studentIds = [
    ...new Set(
      sessions
        .map(
          (s) =>
            (s.booking as { student_user_id: string } | null)?.student_user_id,
        )
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-600">View and manage your coaching sessions</p>
      </div>
          ...
          {upcomingSessions.length > 0 ? (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-medium text-slate-900">
                          {studentMap.get(booking.student_user_id) ||
                            "Unknown Student"}
                        </p>
                        <p className="text-sm text-slate-500">
                          {formatDate(booking.start_at, "EEEE, MMM d")} at{" "}
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-slate-400 mt-1">
                Sessions will appear here when parents book with you
              </p>
            </div>
          )}
        </CardContent>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-500">
                          {formatDate(booking.start_at, "MMM d, yyyy")} at{" "}
                          {formatDate(booking.start_at, "h:mm a")}
                        </p>
                      </div>
                    </div>
