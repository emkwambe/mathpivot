...
async function submitAssessmentAction(formData: FormData) {
  ...
  const academicPerformance = parseInt(
    formData.get("academicPerformance") as string,
    10,
  );
  const mathPassion = parseInt(formData.get("mathPassion") as string, 10);
  const achievementLevel = parseInt(
    formData.get("achievementLevel") as string,
    10,
  );
  const careerDirection = parseInt(
    formData.get("careerDirection") as string,
    10,
  );
  const personalQualities = parseInt(
    formData.get("personalQualities") as string,
    10,
  );
  ...
  await supabase.from("student_eligibility_profiles").upsert(
    {
...
export default async function StudentAssessmentPage({
  ...
  // Get student's name separately to avoid RLS issues with joined tables
  ...
  const studentName = studentUserProfile?.full_name || "Unknown Student";
  ...
  const factors = [
    ...
    },
    {
      ...
      description:
        "Work ethic, persistence, openness to feedback, growth mindset",
      levels: [
        "Needs Work (1)",
        "Developing (2)",
        "Good (3)",
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Grade {student.grade} •{" "}
                  {student.course_track?.replace("_", " ")}
                </p>
              </div>
            </div>
            {eligibility && (
                  ...
                  {[1, 2, 3, 4, 5].map((score) => (
                    ...
                    >
                      ...
                      <div className="p-3 text-center border-2 rounded-lg peer-checked:border-blue-500 peer-checked:bg-blue-50 hover:bg-slate-50 transition-colors">
                        <span className="text-2xl font-bold text-slate-700 peer-checked:text-blue-600">
                          {score}
                        </span>
                        <p className="text-xs text-slate-500 mt-1">
                          {factor.levels[score - 1]?.split(" (")[0]}
                        </p>
                      </div>
                    </label>
                  ))}
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">
                  {eligibility.next_assessment_date || "Not scheduled"}
                </p>
              </div>
            </div>
            {eligibility.notes && (
