  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500">Track your payouts and pay rates</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          ...
          <p className="text-2xl font-bold text-green-600 mt-1">${(totalEarned / 100).toFixed(2)}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Pending</p>
          <p className="text-2xl font-bold text-amber-600 mt-1">${(totalPending / 100).toFixed(2)}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4">
            ...
            {rates.map((r: any) => (
              <div key={r.id} className="flex items-center justify-between text-sm">
                ...
                <span className="text-slate-700">
                  ${(r.rate_cents / 100).toFixed(2)} / {r.rate_type.replace('per_', '')}
                </span>
              </div>
            ))}
          </div>
            ...
            {payouts.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-12 text-center text-slate-400">
                  No payouts yet. Earnings will appear here after your sessions are processed.
                </td>
              </tr>
            )}
            {payouts.map((p: any) => (
              <tr key={p.id}>
                <td className="px-4 py-3 text-slate-900">
                  {p.period ? (
                    <>
                      {new Date(p.period.period_start).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                      {' - '}
                      {new Date(p.period.period_end).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                    </>
                  ) : (
                    'N/A'
                  )}
