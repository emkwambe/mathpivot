...
const AVAILABLE_INTEGRATIONS = [
  {
    ...
    description: 'Sync your tutoring sessions with Google Calendar',
    icon: (
  ...
  },
  {
    ...
    icon: (
      <svg className="w-8 h-8" viewBox="0 0 24 24">
        ...
        <path fill="white" d="M7 8.5h5.5c.83 0 1.5.67 1.5 1.5v4c0 .83-.67 1.5-1.5 1.5H7c-.83 0-1.5-.67-1.5-1.5v-4c0-.83.67-1.5 1.5-1.5zm8.5 1l2.5-1.5v8l-2.5-1.5v-5z"/>
      </svg>
    ),
  ...
  },
  {
    ...
    icon: (
      <svg className="w-8 h-8" viewBox="0 0 24 24">
        ...
        <path fill="white" d="M12 12c1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3 1.34 3 3 3zm0 1.5c-2 0-6 1-6 3v1.5h12v-1.5c0-2-4-3-6-3z"/>
        <path fill="#57BB8A" d="M18 11c.83 0 1.5-.67 1.5-1.5S18.83 8 18 8s-1.5.67-1.5 1.5.67 1.5 1.5 1.5zm0 1c-1.11 0-2.5.5-2.5 1.5v1h5v-1c0-1-1.39-1.5-2.5-1.5z"/>
      </svg>
    ),
...
export default function IntegrationsPage() {
  ...
  useEffect(() => {
    async function loadIntegrations() {
      ...
      const hasClassroom = integrationData?.some(i => i.provider === 'classroom');
      if (hasClassroom) {
  ...
  async function handleConnect(provider: string) {
    ...
    setTimeout(() => {
      alert(`OAuth integration with ${provider} would redirect to the provider's authorization page. This feature requires backend OAuth implementation.`);
      setConnecting(null);
    }, 500);
  }
  ...
  async function toggleCourseSync(courseId: string, enabled: boolean) {
    ...
    setClassroomCourses(prev =>
      prev.map(c => c.id === courseId ? { ...c, sync_enabled: enabled } : c)
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-600">Connect external services to enhance your experience</p>
      </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {integrations.length} of {AVAILABLE_INTEGRATIONS.length} services connected
                </p>
              </div>
            </div>
                          ...
                          <div>
                            {data.email && (
                              <p className="text-slate-700">
                                <span className="text-slate-500">Account:</span> {data.email}
                              </p>
                            )}
          ...
          <CardContent>
            <div className="space-y-3">
              {classroomCourses.map((course) => (
                ...
                >
                  <div>
                    <h4 className="font-medium text-slate-900">{course.course_name || course.course_id}</h4>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <input
                      ...
                      onChange={(e) => toggleCourseSync(course.id, e.target.checked)}
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h4 className="font-medium text-slate-900 mb-1">About Integrations</h4>
              <p className="text-sm text-slate-600">
                Connecting external services allows MathPivot to automatically sync your calendar,
                create video meeting links, and import student data. Your data is encrypted and
