...
function getScoreBadge(score: number, maxScore: number) {
  ...
  if (percentage >= 70)
    return { label: "Satisfactory", variant: "warning" as const };
  if (percentage >= 60)
...
export default async function StudentDiagnosticsPage() {
  ...
  const averageScore = diagnostics?.length
    ? diagnostics.reduce(
        (sum, d) => sum + ((d.score || 0) / (d.max_score || 1)) * 100,
        0,
      ) / diagnostics.length
    : 0;
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-600">
          View your assessment results and progress over time
        </p>
      </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p
                className={`text-3xl font-bold ${averageScore >= 70 ? "text-emerald-600" : "text-amber-600"}`}
              >
          ...
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-lg font-bold text-blue-600">
                {latestDiagnostic
                  ? COURSE_TRACK_LABELS[latestDiagnostic.course_track] ||
                    latestDiagnostic.course_track
                  : "-"}
              </p>
            </div>
          </CardContent>
          ...
          {diagnostics && diagnostics.length > 0 ? (
            <div className="space-y-4">
              {diagnostics.map((diagnostic) => {
                const adminBy = diagnostic.administered_by as {
                  full_name: string;
                } | null;
                const scoreBadge =
                  diagnostic.score && diagnostic.max_score
                        ...
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <div>
                            <h3 className="font-semibold text-slate-900">
                              {COURSE_TRACK_LABELS[diagnostic.course_track] ||
                                diagnostic.course_track}{" "}
                              Diagnostic
                            </h3>
                            <p className="text-sm text-slate-500">
                              {formatDate(diagnostic.administered_at)}
                              {adminBy &&
                                ` • Administered by ${adminBy.full_name}`}
                            </p>
                          </div>
                        </div>
                        ...
                        {diagnostic.score !== null && diagnostic.max_score && (
                          <div className="flex items-center gap-4 mb-3">
                            <div className="flex items-baseline gap-1">
                              <span
                                className={`text-2xl font-bold ${getScoreColor(diagnostic.score, diagnostic.max_score)}`}
                              >
                                {diagnostic.score}
                              </span>
                            ...
                            <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full ${
                                  diagnostic.score / diagnostic.max_score >= 0.8
                                    ? "bg-emerald-500"
                                    : diagnostic.score / diagnostic.max_score >=
                                        0.6
                                      ? "bg-amber-500"
                                ...
                                style={{
                                  width: `${(diagnostic.score / diagnostic.max_score) * 100}%`,
                                }}
                              />
                            </div>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-slate-500">
                Your math coach will administer a diagnostic assessment to help
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">
                Diagnostics help us understand your current math skills and
                place you on the right learning track. They are administered by
                your math coach and cover topics relevant to your grade level
                and goals.
              </p>
            </div>
          </div>
