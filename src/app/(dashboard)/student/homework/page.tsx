...
export default async function StudentHomeworkPage() {
  ...
  const studentHomework = homework?.filter((item) => {
  ...
  const pendingHomework = studentHomework.filter(h => h.status === 'assigned' || h.status === 'in_progress');
  const completedHomework = studentHomework.filter(h => h.status === 'completed' || h.status === 'reviewed');
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <p className="text-3xl font-bold text-yellow-600">{pendingHomework.length}</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <p className="text-3xl font-bold text-red-600">
            {pendingHomework.filter(h => isOverdue(h.due_date, h.status)).length}
          </p>
          <p className="text-sm text-slate-500">Overdue</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <p className="text-3xl font-bold text-slate-900">{studentHomework.length}</p>
      ...
      {/* Pending Section */}
      <div className="mb-8">
        <h2 className="text-lg font-semibold text-slate-900 mb-4">Pending Assignments</h2>
        {pendingHomework.length === 0 ? (
          <div className="bg-white rounded-xl border border-slate-200 p-8 text-center">
            ...
            <p className="text-slate-600">All caught up! No pending homework.</p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Submitted: {item.submitted_at ? formatDate(item.submitted_at) : 'N/A'}
                    </p>
                    {item.review_notes && (
                      <div className="mt-2 p-2 bg-purple-50 rounded-lg">
                        <p className="text-xs font-medium text-purple-800">Tutor Feedback:</p>
                        <p className="text-sm text-purple-700">{item.review_notes}</p>
                      </div>
                    )}
                  </div>
