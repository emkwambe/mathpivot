...
export default async function StudentProgressPage() {
  ...
  const masteryLevelInfo: Record<string, { color: string; bg: string; label: string; progress: number }> = {
    not_started: { color: 'text-slate-600', bg: 'bg-slate-200', label: 'Not Started', progress: 0 },
    developing: { color: 'text-amber-700', bg: 'bg-amber-400', label: 'Developing', progress: 33 },
    proficient: { color: 'text-sky-700', bg: 'bg-sky-400', label: 'Proficient', progress: 66 },
    mastered: { color: 'text-green-700', bg: 'bg-green-400', label: 'Mastered', progress: 100 },
  };

  // Calculate overall stats
  const totalSkills = masteryData?.length || 0;
  const masteredCount = masteryData?.filter((m) => m.current_level === 'mastered').length || 0;
  const proficientCount = masteryData?.filter((m) => m.current_level === 'proficient').length || 0;
  const developingCount = masteryData?.filter((m) => m.current_level === 'developing').length || 0;

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">My Progress</h1>
        <p className="text-slate-600">Track your math skills and mastery levels</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <p className="text-3xl font-bold text-amber-600">{developingCount}</p>
      ...
      {Object.keys(skillsByCategory).length > 0 ? (
                    ...
                    return (
                      <div key={skill?.id} className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <Badge className={`${levelInfo.bg} ${levelInfo.color}`}>
                            {levelInfo.label}
                          </Badge>
                        </div>
                        ...
                        {item.last_assessed_at && (
                          <p className="text-xs text-slate-400">
                            Last updated: {formatDate(item.last_assessed_at, 'MMM d, yyyy')}
                          </p>
                        )}
                      </div>
                    );
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <h3 className="text-lg font-medium text-slate-900 mb-2">No progress tracked yet</h3>
