...
type WaitlistStatus = 'waiting' | 'notified' | 'booked' | 'expired' | 'canceled';

const STATUS_COLORS: Record<WaitlistStatus, 'warning' | 'info' | 'success' | 'secondary' | 'danger'> = {
  waiting: 'warning',
  notified: 'info',
  booked: 'success',
  expired: 'secondary',
...
export default async function AdminWaitlistPage() {
  ...
  const notifiedCount = waitlistEntries?.filter(e => e.status === 'notified').length || 0;
  const bookedCount = waitlistEntries?.filter(e => e.status === 'booked').length || 0;
  ...
  const completedEntries = waitlistEntries?.filter(e => ['booked', 'expired', 'canceled'].includes(e.status)) || [];
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-600">Manage session waitlist requests and convert to bookings</p>
      </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-3xl font-bold text-blue-600">{notifiedCount}</p>
            </div>
          </CardContent>
        </Card>
          ...
          {waitingEntries.length > 0 ? (
            <div className="space-y-4">
              {waitingEntries.map((entry) => {
                ...
                const parent = entry.parent as { full_name: string; email: string } | null;
                const tutorProfile = entry.tutor as { users_profile: { full_name: string } } | null;
                      ...
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            ...
                            <p className="font-medium text-slate-900">{formatDate(entry.preferred_date)}</p>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <p className="font-medium text-slate-900 capitalize">{entry.modality}</p>
                          </div>
                          <div>
                            <p className="text-slate-500">Requested Tutor</p>
                            <p className="font-medium text-slate-900">
                              {tutorProfile?.users_profile?.full_name || 'Any available'}
                            </p>
                          </div>
                        </div>
            ...
            <p className="text-center text-slate-500 py-8">No entries currently waiting.</p>
          )}
        </CardContent>
      </Card>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm text-slate-600">
                        {formatDate(entry.preferred_date)} at {entry.preferred_start_time}
                      </p>
                      <p className="text-xs text-slate-500">Parent: {parent?.full_name}</p>
                    </div>
                  ...
                  {completedEntries.slice(0, 20).map((entry) => {
                    ...
                    return (
                      <tr key={entry.id} className="border-b border-slate-100">
                        ...
                        <td className="py-3 text-slate-600">{formatDate(entry.preferred_date)}</td>
                        <td className="py-3">
                          <Badge variant={STATUS_COLORS[entry.status as WaitlistStatus]}>
                            {entry.status}
                          </Badge>
                        </td>
                        <td className="py-3 text-slate-500">{formatDate(entry.created_at)}</td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        <Card>
          <CardContent className="py-12">
            <div className="text-center">
              ...
              <h3 className="text-lg font-medium text-slate-900 mb-2">No Waitlist Entries</h3>
              <p className="text-slate-500">When parents request sessions that aren't available, they'll appear here.</p>
            </div>
          </CardContent>
        </Card>
      )}
