    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <select
          ...
          onChange={(e) =>
            router.push(`/admin/payroll?period=${e.target.value}`)
          }
          className="border border-slate-300 rounded-lg px-3 py-2 text-sm"
        >
          {periods.length === 0 && <option value="">No periods</option>}
          {periods.map((p: PayoutPeriod) => (
          ...
          <tbody className="divide-y divide-slate-100">
            {payouts.length === 0 && (
              <tr>
                ...
                >
                  {selectedPeriodId
                    ? 'No payouts for this period. Click "Generate Payouts" to calculate.'
                    : "Select or create a payout period."}
                </td>
              </tr>
            )}
