      ...
      {editingId && (() => {
        ...
        return (
          <div className="bg-white border border-slate-200 rounded-xl p-4 mt-4">
            <h3 className="font-medium text-slate-900 mb-3">Edit: {u.full_name}</h3>
            <form action={handleSaveProfile} className="grid grid-cols-2 gap-3">
              <input type="hidden" name="userId" value={u.id} />
              <div>
                <label className="block text-xs text-slate-500 mb-1">Full Name</label>
                <input type="text" name="fullName" defaultValue={u.full_name} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">Phone</label>
                <input type="tel" name="phone" defaultValue={u.phone || ''} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">Timezone</label>
                <input type="text" name="timezone" defaultValue={u.timezone || 'America/New_York'} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
              </div>
