...
export function ImportManager() {
  ...
  return (
    <div className="space-y-4">
      {result && (
        <div
          className={`rounded-lg p-3 ${result.type === "success" ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}
        >
          <p
            className={`text-sm ${result.type === "success" ? "text-green-700" : "text-red-700"}`}
          >
            {result.message}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          ...
          <p className="text-xs text-slate-500 mb-3">
            CSV: full_name, email, grade, course_track, parent_email
          </p>
          <form action={(fd) => handleImport(fd, "students")}>
            ...
            <button
              type="submit"
              disabled={loading}
              className="bg-blue-600 text-white rounded-lg px-4 py-2 text-sm font-medium hover:bg-blue-700 disabled:opacity-50 w-full"
            >
        ...
        {/* Import Coaches */}
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <h3 className="font-semibold text-slate-900 mb-1">Import Coaches</h3>
          <p className="text-xs text-slate-500 mb-3">
            CSV: full_name, email, specialties (;sep), hourly_rate
          </p>
          <form action={(fd) => handleImport(fd, "tutors")}>
            ...
            <button
              ...
              className="bg-blue-600 text-white rounded-lg px-4 py-2 text-sm font-medium hover:bg-blue-700 disabled:opacity-50 w-full"
            >
              {loading ? "Importing..." : "Import Coaches"}
            </button>
          </form>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            disabled={loading}
            className="border border-slate-300 text-slate-700 rounded-lg px-4 py-2 text-sm font-medium hover:bg-slate-50 disabled:opacity-50"
          >
            Export Students CSV
          </button>
          <button
