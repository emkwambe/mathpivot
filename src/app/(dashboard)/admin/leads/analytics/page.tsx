...
export default async function LeadAnalyticsPage() {
  ...
  const totalConverted = funnelData.find(f => f.status === 'converted')?.count || 0;
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500">Conversion tracking and source performance</p>
        </div>
        <Link
          href="/admin/leads"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          ...
          <p className="text-2xl font-bold text-green-600 mt-1">{totalConverted}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 mb-8">
        ...
        {totalLeads === 0 ? (
          <p className="text-slate-400 text-sm text-center py-8">No leads yet. Share your get-started page to start capturing leads.</p>
        ) : (
            ...
            {funnelData.map((stage) => {
              ...
              return (
                <div key={stage.status} className="flex items-center gap-3">
                  ...
                  <span className="w-12 text-sm text-slate-500 text-right">{stage.count}</span>
                </div>
              );
            })}
          </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="text-right px-4 py-3 font-medium text-slate-600">Qualified</th>
              <th className="text-right px-4 py-3 font-medium text-slate-600">Trials</th>
              <th className="text-right px-4 py-3 font-medium text-slate-600">Converted</th>
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {stats.map((row: any) => (
              <tr key={row.source}>
                ...
                <td className="px-4 py-3 text-right text-slate-700">{row.qualified}</td>
                <td className="px-4 py-3 text-right text-slate-700">{row.trials}</td>
                <td className="px-4 py-3 text-right font-medium text-green-600">{row.converted}</td>
                <td className="px-4 py-3 text-right">
                  <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${
                    parseFloat(row.conversionRate) >= 20 ? 'bg-green-100 text-green-700' :
                    parseFloat(row.conversionRate) >= 10 ? 'bg-amber-100 text-amber-700' :
