  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500">Manage coding challenges for students</p>
      </div>
            ...
            {problems.map((p: unknown) => (
              <tr key={p.id}>
                ...
                <td className="px-4 py-3">
                  <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${difficultyColors[p.difficulty] || 'bg-slate-100'}`}>
                    {p.difficulty}
                  </span>
                </td>
                <td className="px-4 py-3 text-slate-700 capitalize">{p.language}</td>
                <td className="px-4 py-3 text-slate-700">{p.topic || '-'}</td>
                <td className="px-4 py-3 text-slate-700">{(p.test_cases as any[])?.length || 0}</td>
              </tr>
            ))}
          </tbody>
        </table>
