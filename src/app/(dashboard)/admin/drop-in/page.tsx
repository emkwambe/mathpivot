  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Drop-In Sessions</h1>
          <p className="text-sm text-slate-500">Manage open lab / walk-in tutoring slots</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Active Slots</p>
          <p className="text-2xl font-bold text-slate-900 mt-1">{slots.length}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Currently Checked In</p>
          <p className="text-2xl font-bold text-blue-600 mt-1">{checkedIn.length}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Total Today</p>
          <p className="text-2xl font-bold text-green-600 mt-1">{attendance.length}</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden mb-6">
        ...
        {slots.length === 0 ? (
          <div className="p-8 text-center text-slate-400 text-sm">No drop-in slots configured.</div>
        ) : (
            ...
            <tbody className="divide-y divide-slate-100">
              {slots.map((slot: unknown) => (
                <tr key={slot.id}>
                  ...
                  <td className="px-4 py-3 text-slate-700">{slot.start_time} - {slot.end_time}</td>
                  <td className="px-4 py-3 text-slate-700">{slot.tutor?.full_name || '-'}</td>
                  <td className="px-4 py-3 text-slate-700">{slot.room?.name || slot.modality}</td>
                  <td className="px-4 py-3 text-slate-700">{slot.max_concurrent}</td>
                </tr>
              ))}
            </tbody>
          </table>
