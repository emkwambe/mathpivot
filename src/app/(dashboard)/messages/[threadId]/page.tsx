...
export default async function ThreadPage({ params }: Props) {
  ...
  const { data: thread } = await supabase
    ...
    .select(
      "id, participant_a, participant_b, student_user_id, subject, status",
    )
    .eq("id", threadId)
    .single();
  ...
  const studentProfile = thread.student_user_id
    ? profileMap.get(thread.student_user_id)
    : null;
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] max-w-4xl mx-auto">
      ...
      {/* Composer */}
      {thread.status === "active" && <MessageComposer threadId={threadId} />}

      {thread.status !== "active" && (
        <div className="p-4 bg-slate-100 border-t border-slate-200 rounded-b-xl text-center">
          <p className="text-sm text-slate-500">
            This conversation has been {thread.status}.
          </p>
        </div>
      )}
    </div>
