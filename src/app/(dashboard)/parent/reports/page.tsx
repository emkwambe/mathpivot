...
export default async function ParentReportsPage() {
    ...
    return (
      <div className="p-6 max-w-4xl mx-auto">
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-8 text-center">
          <h2 className="text-lg font-semibold text-yellow-800">No Family Associated</h2>
          <p className="text-yellow-600 mt-2">Please contact support to set up your family account.</p>
        </div>
      </div>
    );
  }
  ...
  const { data: reports } = await supabase
    .from('weekly_reports')
    .select('*')
    .in('student_user_id', studentIds.length > 0 ? studentIds : ['00000000-0000-0000-0000-000000000000'])
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-600">Progress updates from your students&apos; tutors</p>
      </div>
      ...
      {(!reports || reports.length === 0) ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          ...
          <p className="text-slate-500">Weekly reports will appear here after tutoring sessions are completed.</p>
        </div>
      ) : (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="text-right">
                  <p className="text-2xl font-bold text-slate-900">{report.sessions_count}</p>
              ...
              <div className="grid grid-cols-3 gap-4 mb-4 p-3 bg-slate-50 rounded-lg">
                <div className="text-center">
                  <p className="text-lg font-semibold text-slate-900">
                    {report.attendance_rate ? `${Math.round(report.attendance_rate * 100)}%` : 'N/A'}
                  </p>
                ...
                <div className="text-center">
                  <p className="text-lg font-semibold text-slate-900">
                    {Array.isArray(report.mastery_changes) ? report.mastery_changes.length : 0}
                  </p>
              ...
              {report.summary_text && (
                <div className="mb-3">
                  ...
                  <p className="text-sm text-slate-600">{report.summary_text}</p>
                </div>
              )}
              ...
              {/* At Risk Reasons */}
              {report.is_at_risk && report.at_risk_reasons && report.at_risk_reasons.length > 0 && (
                <div className="p-3 bg-red-50 rounded-lg">
                  <p className="text-sm font-medium text-red-800 mb-1">Areas of Concern</p>
                  <ul className="list-disc list-inside text-sm text-red-700">
                    {report.at_risk_reasons.map((reason: string, i: number) => (
                      <li key={i}>{reason}</li>
                    ))}
                  </ul>
                </div>
              ...
              {/* Skills Practiced */}
              {report.skills_practiced && report.skills_practiced.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {report.skills_practiced.slice(0, 5).map((skill: string, i: number) => (
                    <span
                      key={i}
