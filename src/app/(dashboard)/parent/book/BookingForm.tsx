  ...
  const handleSubmit = async () => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        setBookingSuccess({
          tutorName: selectedTutorData?.name || "Coach",
          studentName:
    ...
    return (
      <div className="text-center py-8 space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-medium text-slate-900">
                  ...
                  at{" "}
                  {bookingSuccess.startAt.toLocaleTimeString("en-US", {
                    hour: "numeric",
                    minute: "2-digit",
                    hour12: true,
                  })}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          <AddToCalendar
            title={`Math Coaching: ${bookingSuccess.studentName}`}
            description={`Coaching session with ${bookingSuccess.tutorName}`}
        ...
        <p className="text-sm text-slate-500">
          The tutor will confirm your session shortly. You&apos;ll receive a
    ...
    <div className="space-y-6">
            ...
            {suggestions.slice(0, 3).map((suggestion, index) => (
              ...
              >
                ...
                <p className="text-sm text-slate-700">
                  with{" "}
                  <span className="font-medium">{suggestion.tutorName}</span>
                </p>
              </button>
            ))}
          </div>
        ...
        {steps.map((s, index) => (
          <div key={s.number} className="flex items-center flex-1">
            <div className="flex flex-col items-center">
              ...
              <span
                className={`text-xs mt-2 ${currentStep >= s.number ? "text-blue-600 font-medium" : "text-slate-400"}`}
              >
                {s.label}
              </span>
            </div>
            {index < steps.length - 1 && (
              <div
                className={`flex-1 h-0.5 mx-2 ${currentStep > s.number ? "bg-blue-600" : "bg-slate-200"}`}
              />
            )}
          </div>
        ))}
      ...
      {selectedSlot && (
        <div className="space-y-4 border-t-2 border-slate-100 pt-6">
          ...
          <p className="text-xs text-slate-500 text-center">
            By booking, you agree to our cancellation policy. Late cancellations
            (less than 24 hours) may forfeit the session credit.
          </p>
        </div>
      )}
    </div>
