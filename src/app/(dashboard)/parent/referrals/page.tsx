...
import { getMyReferralCode, getMyReferrals, getRewardPolicy } from '@/app/actions/referrals';
      ...
      {policy && (
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-6 text-white mb-6">
          ...
          <div className="flex gap-6">
            <div>
              <p className="text-blue-200 text-xs uppercase tracking-wide">You earn</p>
              <p className="text-2xl font-bold">${(policy.referrer_reward_cents / 100).toFixed(0)}</p>
            </div>
            <div>
              <p className="text-blue-200 text-xs uppercase tracking-wide">They get</p>
              <p className="text-2xl font-bold">${(policy.referred_reward_cents / 100).toFixed(0)}</p>
            </div>
            <div>
              <p className="text-blue-200 text-xs uppercase tracking-wide">You&apos;ve earned</p>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm text-slate-600">
              <p>{stats?.total_referrals || 0} referred &middot; {stats?.total_conversions || 0} converted</p>
            </div>
          </div>
        </div>
      )}
              ...
              {referrals.map((ref: any) => (
                <tr key={ref.id}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[ref.status] || 'bg-slate-100'}`}>
                      {ref.status.replace('_', ' ')}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-slate-700">
                    {ref.referrer_reward_cents > 0 ? `$${(ref.referrer_reward_cents / 100).toFixed(2)}` : '-'}
                  </td>
