...
const LEAD_STATUSES = ['new', 'contacted', 'qualified', 'trial_scheduled', 'trial_completed', 'converted', 'lost'] as const;
const LEAD_SOURCES = ['website', 'referral', 'social_media', 'google_ads', 'facebook_ads', 'walk_in', 'phone', 'email', 'partner', 'event', 'other'] as const;
...
export async function captureLeadAction(
  ...
  const subjects = parsed.data.subjectsInterested
    ? parsed.data.subjectsInterested.split(',').map(s => s.trim()).filter(Boolean)
    : [];
...
// ============================================================
export async function updateLeadStatus(leadId: string, newStatus: string): Promise<LeadResult> {
  const supabase = await createClient();
...
// ============================================================
export async function updateLead(leadId: string, formData: FormData): Promise<LeadResult> {
  const supabase = await createClient();

  const updates: Record<string, any> = {};
  const fields = ['parent_name', 'parent_email', 'parent_phone', 'student_name', 'goals', 'notes', 'budget_range', 'preferred_schedule', 'lost_reason'];
  for (const field of fields) {
    const val = formData.get(field);
    if (val !== null) updates[field] = val || null;
  }

  const grade = formData.get('student_grade');
  if (grade) updates.student_grade = parseInt(grade as string);

  const assignedTo = formData.get('assigned_to');
  if (assignedTo !== null) updates.assigned_to = assignedTo || null;

  const nextFollowUp = formData.get('next_follow_up_at');
...
// ============================================================
export async function addLeadNote(leadId: string, note: string): Promise<LeadResult> {
  const supabase = await createClient();
...
export async function getLeadDetail(leadId: string) {
  ...
  const [leadResult, activitiesResult, trialsResult] = await Promise.all([
    supabase.from('leads').select('*, assigned_user:assigned_to (full_name, email)').eq('id', leadId).single(),
    supabase.from('lead_activities').select('*').eq('lead_id', leadId).order('created_at', { ascending: false }).limit(50),
    supabase.from('trial_lessons').select('*').eq('lead_id', leadId).order('scheduled_at', { ascending: false }),
  ]);
...
export async function scheduleTrialLesson(formData: FormData): Promise<LeadResult> {
  const supabase = await createClient();
...
export async function completeTrialLesson(
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      tutor_rating: formData.get('tutorRating') ? parseInt(formData.get('tutorRating') as string) : null,
      recommended_track: formData.get('recommendedTrack') as string || null,
      recommended_package: formData.get('recommendedPackage') as string || null,
...
export async function getLeadAnalytics() {
  ...
  // Build source stats
  const sourceMap = new Map<string, { total: number; qualified: number; trials: number; converted: number }>();
  for (const lead of leads) {
    const s = sourceMap.get(lead.source) || { total: 0, qualified: 0, trials: 0, converted: 0 };
    s.total++;
    if (lead.status !== 'new' && lead.status !== 'lost') s.qualified++;
    if (['trial_scheduled', 'trial_completed', 'converted'].includes(lead.status)) s.trials++;
    if (lead.status === 'converted') s.converted++;
    sourceMap.set(lead.source, s);
  }

  const stats = Array.from(sourceMap.entries()).map(([source, data]) => ({
    source,
    ...data,
