...
export async function createPayoutPeriod(formData: FormData): Promise<PayrollResult> {
  const supabase = await createClient();
...
// ============================================================
export async function generatePayoutsForPeriod(periodId: string): Promise<PayrollResult> {
  ...
  if (periodError || !period) return { success: false, error: 'Period not found' };
  if (period.is_locked) return { success: false, error: 'Period is already locked' };
  ...
  if (!tutors || tutors.length === 0) return { success: false, error: 'No active tutors found' };

  let generated = 0;

  for (const tutor of tutors) {
    // Calculate earnings using RPC
    const { data: calc, error: calcError } = await admin.rpc('calculate_tutor_payout', {
    ...
    if (calcError) {
      console.error(`[payroll] calc error for ${tutor.user_id}:`, calcError.message);
      continue;
    }
    ...
    if (upsertError) {
      console.error(`[payroll] upsert error for ${tutor.user_id}:`, upsertError.message);
      continue;
    }
  ...
  revalidatePath('/admin/payroll');
  return { success: true, error: generated === 0 ? 'No sessions found for any tutor in this period' : undefined };
}
...
export async function setTutorPayRate(formData: FormData): Promise<PayrollResult> {
  const supabase = await createClient();
