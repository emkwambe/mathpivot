...
export type CodeResult = { success: boolean; error?: string; submissionId?: string };
...
export async function getCodeProblems(topic?: string) {
  ...
  let query = supabase
    .from('code_problems')
    .select('id, title, description, difficulty, language, topic, starter_code, hints, time_limit_ms')
    .eq('is_active', true)
...
export async function getProblemDetail(problemId: string) {
  ...
  if (error || !data) return { problem: null, error: error?.message || 'Not found' };
...
const submitSchema = z.object({
  ...
  language: z.enum(['python', 'javascript', 'java', 'cpp', 'r', 'sql', 'pseudocode']),
...
export async function createCodeProblem(formData: FormData): Promise<CodeResult> {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
