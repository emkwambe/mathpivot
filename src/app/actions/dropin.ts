...
export async function createDropInSlot(
  formData: FormData,
): Promise<DropInResult> {
  ...
  const { error } = await supabase.from("drop_in_slots").insert({
...
export async function checkInStudent(slotId: string): Promise<DropInResult> {
  ...
  const { error } = await supabase.from("drop_in_attendance").insert({
    slot_id: slotId,
