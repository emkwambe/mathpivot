...
interface SessionExportRow {
  ...
  booking: { start_at: string; end_at: string; modality: string } | null;
}
...
export async function exportStudentsCSV(): Promise<{
  ...
  if (!students || students.length === 0)
    return { csv: "", error: "No students found" };

  const headers = "full_name,email,role,created_at";
  const rows = students.map(
    (s) => `"${s.full_name}","${s.email}","${s.role}","${s.created_at}"`,
  );
  return { csv: [headers, ...rows].join("\n") };
}

export async function exportTutorsCSV(): Promise<{
  csv: string;
  error?: string;
}> {
  const supabase = await createClient();
  ...
  const tutors = (data || []) as unknown as TutorExportRow[];
  if (tutors.length === 0) return { csv: "", error: "No tutors found" };

  const headers = "full_name,email,specialties,hourly_rate_cents,is_active";
  const rows = tutors.map(
    (t) =>
      `"${t.user?.full_name || ""}","${t.user?.email || ""}","${(t.specialties || []).join(";")}",${t.hourly_rate || 0},${t.is_active}`,
  );
  return { csv: [headers, ...rows].join("\n") };
}
...
export async function exportSessionsCSV(
  ...
  const sessions = (data || []) as unknown as SessionExportRow[];
  if (sessions.length === 0) return { csv: "", error: "No sessions found" };

  const headers =
    "session_id,status,attendance,start_at,end_at,modality,completed_at";
  const rows = sessions.map(
    (s) =>
      `"${s.id}","${s.status}","${s.attendance_status || ""}","${s.booking?.start_at || ""}","${s.booking?.end_at || ""}","${s.booking?.modality || ""}","${s.completed_at || ""}"`,
  );
  return { csv: [headers, ...rows].join("\n") };
}
