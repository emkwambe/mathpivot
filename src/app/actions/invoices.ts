...
export async function createInvoiceForFamily(
  ...
  const { data: invNumber, error: numError } = await admin.rpc(
...
export async function generateSessionInvoice(
  ...
  const { data: sessions } = (await admin
    ...
    .lte("completed_at", dateTo)) as { data: SessionWithBooking[] | null };

  // Filter to this family's sessions
  const familySessions = (sessions || []).filter(
    (
      s: SessionWithBooking,
    ): s is SessionWithBooking & {
      booking: NonNullable<SessionWithBooking["booking"]>;
    } => s.booking?.family_id === familyId,
  );

  if (familySessions.length === 0)
    return { success: false, error: "No billable sessions found" };
  ...
  const lineItems: {
    ...
    const hours =
      (new Date(s.booking.end_at).getTime() -
        new Date(s.booking.start_at).getTime()) /
      3600000;
    const tutorName = nameMap.get(s.booking.tutor_user_id) || "Tutor";
    const studentName = nameMap.get(s.booking.student_user_id) || "Student";
    const date = new Date(s.booking.start_at).toLocaleDateString("en-US", {
      month: "short",
      day: "numeric",
    });
