...
export async function addStudentToFamily(
  ...
  if (authError) {
    if (authError.message.includes("already been registered")) {
      return {
        success: false,
        error:
          "This email is already registered. Contact support if this is your student.",
      };
    }
  ...
  // Request required COPPA consents for the new student
  try {
    const required = await getRequiredConsents(newUser.user.id);
    for (const consentType of required) {
      const alreadyValid = await hasValidConsent(newUser.user.id, consentType);
      if (!alreadyValid) {
        await requestConsent(newUser.user.id, user.id, consentType);
      }
    }
  } catch (err) {
    console.error("[consent] Failed to request consents:", err);
    // Don't block student creation — consent can be requested later
  }
